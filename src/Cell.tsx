...
import React, { useRef, useState, useEffect, MutableRefObject, MouseEvent, KeyboardEvent, FocusEvent, ChangeEvent } from 'react'
...
function useMeasureText (ref: MutableRefObject<HTMLElement | null>, text: string): string | undefined {
  const [title, setTitle] = useState<string | undefined>(undefined)
  useEffect(() => {
    if (text !== '' && ref.current && ctx !== null) {
      const measurements = ctx.measureText(text)
      const textIsBiggerThanInput: boolean = measurements.width > ref.current.offsetWidth
...
function Cell (props: CellProps) {
  ...
  const [stateValue, setValue] = useState<string | null>(null)
  ...
  return (
    <div
      ref={tdRef}
      title={stateValue === null && title ? title : undefined}
      tabIndex={0}
      className={classnames('spreadsheet__cell', className, {
        'txt-ellipsis': stateValue === null && title,
      })}
      onMouseDownCapture={handleMouseDown}
      onDoubleClick={focusInput}
      onKeyDown={handleKeyDown}>{ children(inputRef, propsToPass) }</div>
  )
}
